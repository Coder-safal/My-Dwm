...
static void pillar(Monitor *m); //Leyuskc added
...
applyrules(Client *c)
{
	...
	for (i = 0; i < LENGTH(rules); i++) {
      ...
      //Added By Me
			/*Dont switch view for scratch pads*/
      Arg a = {.ui= r->tags};
			view(&a);
			//Ends here
...
spawn(const Arg *arg)
{
	if (fork() == 0) {
		...
		setsid();
		int status_code = execvp(((char **)arg->v)[0], (char **)arg->v);
    if(status_code<0){
		  FILE *fp = fopen("/tmp/dwm_spwan.errors","a");
      fprintf(fp,"[AGENT] dwm: execvp '%s' failed:\n", ((char **)arg->v)[0]);
      fprintf(fp,"\t[REASON] '%s' \n", strerror(errno));
      fclose(fp);
    }
