    ...
    {"󰁹"},
		...
		if (num_perc<20 && sent==0){
			fp = popen("dunstify 'Low Battery' 'Charge Now' -u critical -t 1500 -r 369987 \
				-i ~/Pictures/icons/low-battery.png", "r");
			sent=1;
			bck = num_perc;
			pclose(fp);
		}
		...
		if (num_perc==100 && sent3==0){
			fp = popen("dunstify 'Battery FULL'  -u low -t 1500 -r 369987 \
				-i ~/Pictures/icons/full-battery.png", "r");
			sent3=1;
			bck = num_perc;
			pclose(fp);
		}
		...
		if ((!strcmp("Charging",state)) && (chrg==0)){
			fp = popen("dunstify 'Charging'  -u low -t 1500 -r 369987 \
				-i ~/Pictures/icons/charging-battery.png", "r");
			chrg=1;
		...
		if(i == LEN(map) &&!sent2){
			fp = popen("dunstify 'Battery Error' 'Check Now' -u critical -t 5500 -r 369987 \
				-i ~/Pictures/icons/error-battery.png", "r");
			sent2=1;
			pclose(fp);
		}
        if (i < LEN(map) - 1) {
          return bprintf("%s %d%%", map[i].symbol,num_perc);
        }
        // return  ;
